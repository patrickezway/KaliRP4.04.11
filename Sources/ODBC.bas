...
Public Function Odbc_Init(ByVal v_stypebdd As String, _
    ...
    If Odbc_type_base = ODBC_BDD_PG Then
        ...
        If Driver = Empty Then ' le driver postgreSQL n'est pas installé
            Call LOG_WARN("Odbc_Init", "Driver non installé, création user DSN")
...
err_connection:
    If v_mode_verbeux Then
        Call MsgBox("ODBC_Init " & Err.Number & " " & Err.Description)
    End If
    Call LOG("Odbc_Init", "Connexion à " & v_nombdd, Err)
    Odbc_Init = P_ERREUR
    Exit Function
...
Public Function Odbc_Init_OLD(ByVal v_stypebdd As String, _
    ...
    If P_MODE_DEBUG And v_stypebdd = "PGX" Then
        ...
        If database = "" Then
            database = InputBox("Database", "Connexion à une base de donnée (" & v_nombdd & ")", "kalidoc")
        End If
        Server = SYS_GetIni("DATABASE", "SERVEUR", p_nomini)
        If Server = "" Then
            Server = InputBox("Serveur", "Connexion à une base de donnée (" & v_nombdd & ")", "192.168.101.")
        End If
        Port = SYS_GetIni("DATABASE", "PORT", p_nomini)
        If Port = "" Then
            Port = InputBox("Port", "Connexion à une base de donnée (" & v_nombdd & ")", "5432")
        End If
        Driver = SYS_GetIni("DATABASE", "DRIVER", p_nomini)
        If Driver = "" Then
            Driver = InputBox("Driver", "Connexion à une base de donnée (" & v_nombdd & ")", "PostgreSQL")
        End If
        username = SYS_GetIni("DATABASE", "USERNAME", p_nomini)
        If username = "" Then
            username = InputBox("Username", "Connexion à une base de donnée (" & v_nombdd & ")", "postgres")
        End If
        Password = SYS_GetIni("DATABASE", "PASSWORD", p_nomini)
        If Password = "" Then
            Password = InputBox("Password", "Connexion à une base de donnée (" & v_nombdd & ")", "postgres")
        End If
        p_nomBDD_ODBC = SYS_GetIni("Base", "NOM", p_nomini)
        If p_nomBDD_ODBC = "" Then
            p_nomBDD_ODBC = InputBox("p_nomBDD_ODBC", "Connexion à une base de donnée (" & v_nombdd & ")", "kalidoc")
        End If
        p_nomBDD_SERVEUR = SYS_GetIni("Base", "Nom_BDD_SERVEUR", p_nomini)
        If p_nomBDD_SERVEUR = "" Then
            p_nomBDD_SERVEUR = InputBox("p_nomBDD_SERVEUR", "Connexion à une base de donnée (" & v_nombdd & ")", "kalidoc")
        End If
