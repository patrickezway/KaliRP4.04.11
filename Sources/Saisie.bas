...
' indique s'il y a des champs à saisir ou seulement des boutons
...
Public Type SAIS_SCHAMP
    libelle As String
    len_saisie As Integer  'Longueur du texte à saisir
    len_ecran As Integer  'Longueur du texte à saisir
    type As Integer
...
Public Sub SAIS_AddChamp(ByVal v_libelle As String, _
                         ByVal v_len_saisie As Integer, _
                         ByVal v_len_ecran As Integer, _
    ...
    SAIS_Saisie.champs(n).len_saisie = v_len_saisie
    SAIS_Saisie.champs(n).len_ecran = v_len_ecran
...
Public Function SAIS_CtrlChamp(ByRef vr_str As String, _
    ...
    Case SAIS_TYP_PERIODE
        ...
        If s = "J" Or s = "S" Or s = "M" Or s = "A" Then
            ...
            End If
        End If
        MsgBox "La saisie ne correspond pas à une période : nombre suivi de J(ours)/S(emaines)/M(ois)/A(nnées).", vbOKOnly + vbExclamation, "Saisie Erronnée"
        SAIS_CtrlChamp = False
        Exit Function
    Case SAIS_TYP_HEURE
    ...
    Case SAIS_TYP_CODE
        For i = 1 To Len(vr_str)
            ...
            End If
            If Not fok Then
                MsgBox "Seuls les chiffres, lettres (sauf les caractères accentués) et - _ . sont autorisés.", vbOKOnly + vbExclamation, "Saisie Erronnée"
                SAIS_CtrlChamp = False
                Exit Function
            End If
        Next i
