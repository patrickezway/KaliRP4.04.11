...
'    Else
    ...
    If val(taille) > p_HTTP_MaxParFichier Then
        ...
        nbSecondeAttente = 60 ' nombre de seconde ou on attend
        DateDébut = DateTime.Now()
        If p_bool_HTTP_Fichiers_Multiples Then
            b_attendre = True
LabAttendre:
            ' on attend si pas dépassé le temps
            If DateDiff("s", DateDébut, DateTime.Now()) > nbSecondeAttente Then
                ' délai dépassé
                iret = HTTP_PUT_ERREUR
                If g_HTTP_VoirProgression Then
                    g_HTTP_FormMaj.lblMaj.Caption = "La copie a échoué (" & g_HTTP_FormMaj.lblMaj.Caption & ")"
                    Call MsgBox(g_HTTP_FormMaj.lblMaj.Caption & Chr(13) & Chr(10) & "Fichier non re-transféré vers le serveur")
            ...
            While b_attendre
                For i = 0 To UBound(p_tbl_HTTP_Fichiers_Multiples)
                    b_attendre = False
                    If Not p_tbl_HTTP_Fichiers_Multiples(i).HTTP_Chargé Then
                        b_attendre = True
                        If g_HTTP_VoirProgression Then
                            g_HTTP_FormMaj.lblMaj.Caption = "Reconstruction : En attente du paquet n° " & p_tbl_HTTP_Fichiers_Multiples(i).HTTP_Numero
                        End If
                        DoEvents
                        GoTo LabAttendre
                    End If
...
Err_FSO:
    ...
    r_liberr = "Erreur générée par Scripting.FileSystemObject : " & Error$
    HTTP_PutDos = HTTP_PUTDOS_ERREUR
End Function
