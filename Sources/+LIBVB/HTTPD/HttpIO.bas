...
Public Function HTTP_dupliquerDos(ByVal v_sURL As String, _
                               ByVal v_CheminDossier_Origine As String, _
                               ByVal v_CheminDossier_Destinataire As String, _
                               ByVal v_Trait As String, _
                               ByRef r_liberr As String) As Long
    ...
    ret = http_InitConnect(v_sURL, http_req)
    If CBool(ret) = False Then
        HTTP_dupliquerDos = HTTP_TAILLE_ERREUR
        Exit Function
    End If
    ...
    stLoad = "Content-Type: application/x-www-form-urlencoded" & vbCrLf
    stPost = "v_CheminHTTP=" & p_HTTP_CheminDepot & "&" _
            & "v_CheminDossier_O=" & v_CheminDossier_Origine & "&" _
            & "v_CheminDossier_D=" & v_CheminDossier_Destinataire & "&" _
            & "v_NumUtil=" & v_Trait & p_NumUtil
            
    ret = HttpSendRequest(http_req.lHttpRequest, stLoad, Len(stLoad), stPost, Len(stPost))
    If ret = 0 Then
        Call http_CloseConnect(http_req)
        r_liberr = "DupliquerDossier : HttpSendRequest=0 : Apache arrêté ?"
        HTTP_dupliquerDos = HTTP_TAILLE_ERREUR
        Exit Function
    End If
    ...
    If bresult = False Then
        Call http_CloseConnect(http_req)
        r_liberr = "DupliquerDossier : Erreur HttpQueryInfo sret=" & left(Trim(buf), 3)
        HTTP_dupliquerDos = HTTP_TAILLE_ERREUR
        Exit Function
    End If
...
Public Function HTTP_Appel_DupliquerDos(ByVal v_NomDosSrc As String, _
                                  ByVal v_NomDosDest As String, _
                                  ByVal v_Trait As String, _
                                  ByRef r_liberr As String) As Integer
    ...
    strHTTP = "http://" & p_AdrServeur & "/TRSF_HTTP/dupliquer_dossier.php?v_Trait=" & v_Trait
    'Session = HTTP_RandomAlphaNumString(5)
    
    iret = HTTP_dupliquerDos(strHTTP, v_NomDosSrc, v_NomDosDest, v_Trait, r_liberr)
    
    HTTP_Appel_DupliquerDos = iret
...
Public Function HTTP_Ecrire_Log(ByVal v_Nomlog As String, _
                                    ByVal v_Fonction As String, _
                                    ByVal v_message As String, _
                                    ByRef r_liberr As String) As Integer
    ...
    strHTTP = "http://" & p_AdrServeur & "/TRSF_HTTP/ecrire_log.php"
    
    iret = http_ecrirelog(strHTTP, v_Nomlog, v_Fonction, v_message, r_liberr)
        
    HTTP_Ecrire_Log = iret
    
End Function

Public Function http_ecrirelog(ByVal v_sURL As String, _
                              ByVal v_Nomlog As String, _
                              ByVal v_Fonction As String, _
                              ByVal v_message As String, _
                              ByRef r_liberr As String) As Integer
    Dim bresult As Boolean, bret As Boolean
    ...
    ' Initialise Connect
    v_sURL = v_sURL & "?v_Nomlog=" & v_Nomlog _
            & "&v_Fonction=" & v_Fonction _
            & "&v_Message=" & v_message

    ret = http_InitConnect(v_sURL, http_req)
    If CBool(ret) = False Then
        http_ecrirelog = HTTP_ERREUR
        Exit Function
    End If
    ...
    If ret = 0 Then
        Call http_CloseConnect(http_req)
        r_liberr = "ecrirelog : HttpSendRequest=0 : Apache arrêté ?"
        http_ecrirelog = HTTP_ERREUR
        Exit Function
    End If
    ...
    If bresult = False Then
        Call http_CloseConnect(http_req)
        r_liberr = "ecrirelog : Erreur HttpQueryInfo "
        http_ecrirelog = HTTP_ERREUR
        Exit Function
    End If
    If val(left(Trim(buf), 1)) > 3 Then
        Call http_CloseConnect(http_req)
        r_liberr = "ecrirelog : Erreur HttpQueryInfo sret=" & left(Trim(buf), 3)
        http_ecrirelog = HTTP_ERREUR
        Exit Function
    End If
