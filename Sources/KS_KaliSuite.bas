...
Public Function P_UtilAPlusieursFonctions(ByVal v_numutil As Long) As Integer
    ...
    If Odbc_Select("select U_FctTrav from Utilisateur where U_Num=" & v_numutil, rs) = P_ERREUR Then
...
Public Function P_UtilAPlusieursPostes(ByVal v_numutil As Long) As Integer
    ...
    If Odbc_RecupVal("select U_SPM from Utilisateur where U_Num=" & v_numutil, s_sp) = P_ERREUR Then
